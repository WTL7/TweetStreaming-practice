...
csvFile = open('Twitter_IL_IN_OH_%s.csv'%current_time, 'wb')
#Use csv Writer
    ...
    def on_status(self, status):
        ...
        if any(keyword in status.text.lower() for keyword in keywords): 
            if status.geo is not None:  
                ...
                print "IL_IN_OH ",status.created_at,status.text
...
sapi = tweepy.streaming.Stream(auth, CustomStreamListener())  
    # IL IN OH 
sapi.filter(locations=[-91.20,36.59,-80.39,44.05])  # Each bounding box should be specified as a pair of longitude and latitude pairs, with the southwest corner of the bounding box coming first
